function init() {
    ...
    let qaList = [
        {
            question: "如果有一天我们可以去任何地方度假，你会选择：",
            options: ['热带海岛', '雪山滑雪', '古城旅行', '漫游大自然'],
            type: 'radio'
        },
        {
            question: "如果我们可以一起参加一项挑战或冒险活动，你会选：",
            options: ['极限运动（如蹦极、跳伞等）', '密室逃脱', '徒步穿越', '烹饪大赛'],
            type: 'radio'
        },
        {
            question: "你觉得我最最最最最最最吸引你的地方是：",
            options: ['帅气的外貌', '幽灭', '肥而不腻的性格', '溢出的才华'],
            type: 'radio'
        },
        {question: "你最喜欢我做的哪件事，让你觉得特别温暖或开心？", options: [], type: 'input'},
        {question: "(づ′▽`)づ到这只兔兔就先亲一个吧", options: [], type: 'input'},
        {question: "你觉得我们的第一次约会是什么样子的？", options: [], type: 'input'},
        {
            question: "想要哪件生日礼物?",
            options: ['小米su7 max 创始版', '99999999玫瑰', '神秘礼物A', '神秘大奖B'],
            type: 'checkbox'
        },
        {question: "生日快乐(此设置支付宝口令红包)", options: [], type: 'redBox'},
        {question: "(￣^￣)快夸夸你老公", options: [], type: 'input'},
    ]
    ...
    const updateSadBunnyCount = () => {
        ...
        const questionFlag = sadBunnyCount % 3 == 0
        if (questionFlag) {
...
// 禁用点击事件
    ...
    new Array(27).fill('').forEach(() => addBunny())
